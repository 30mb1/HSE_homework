...
def compute_bias_variance(regressor, dependence_fun, x_generator=np.random.uniform, noise_generator=np.random.uniform,
    ...
    * Проверить правильность реализации можно на примерах, которые разбирались на семинаре и в домашней работе.
    ...
    #x = x.reshape(len(x), 1)
    y = dependence_fun(x)
    noise_sample = noise_generator(size=sample_size)
    y_noise = y + noise_sample

    noise_objects = noise_generator(size=objects_num)
    Eyx = y + np.mean(noise_objects)
    ...
    for i in range(samples_num):
        ...
        noise_sample = noise_generator(size=sample_size)
        y_train = dependence_fun(X_train) + noise_sample

        regressor.fit(X_train[:, np.newaxis], y_train)
        result.append(regressor.predict(x[:, np.newaxis]))

    Emux = sum(result) / float(samples_num)
    
    bias = np.mean((Emux - Eyx) ** 2, axis=0)

    variance = np.mean(np.mean((np.array(result) - Emux) ** 2, axis=0), axis=0)
