...
def compute_bias_variance(regressor, dependence_fun, x_generator=np.random.uniform, noise_generator=np.random.uniform,
                          sample_size=300, samples_num=300, objects_num=400, seed=1234):
    """
    ...
    * Проверить правильность реализации можно на примерах, которые разбирались на семинаре и в домашней работе.
    ...
    x = x_generator(size=objects_num).reshape(-1, 1)
    y = dependence_fun(x).reshape(-1, 1)
    noise_mean = noise_generator(size=objects_num).mean()


    X = x_generator(size=(samples_num, sample_size))
    noise = noise_generator(size=(samples_num, sample_size))
    return compute_bias_variance_fixed_samples(regressor, dependence_fun, X, x, noise, noise_mean)
    pass
...
def compute_bias_variance_fixed_samples(regressor, dependence_fun, samples, objects, noise, mean_noise):
    ...
    * $\mathbb{E}[y|x]$ оценивается как сумма правильного ответа на объекте и мат. ожидания шума
    ...
    :return variance: разброс алгоритма regressor (число)
    """
    prediction = np.empty(samples.shape)
    for i in range(len(samples)):
        regr = regressor
        #print(samples[i].reshape(-1, 1).shape)
       # print(noise[i].shape)
        regr.fit(samples[i].reshape(-1, 1), dependence_fun(samples[i]) + noise[i])
        if i == 0:
            prediction = regr.predict(objects)
        else:
            prediction = np.vstack((prediction, regr.predict(objects)))

    bias = ((prediction.mean(axis=0) - dependence_fun(objects).transpose() - mean_noise) ** 2).mean()
    variance = np.var(prediction, axis=0).mean()
...
def find_best_split(feature_vector, target_vector):
    ...
    * За наличие в функции циклов балл будет снижен. Векторизуйте! :)
    ...
    sorted_features[::2] *= -1
    thresholds = np.diff(sorted_features)
    thresholds = np.divide(thresholds, 2) 
    thresholds[1::2] *= -1 #array of thresholds
    

    feature_vector = feature_vector.reshape(1, feature_vector.size)
    feature = np.tile(feature_vector, (len(thresholds), 1))
    thresholds = np.array(thresholds).reshape(-1, 1)
    target = np.tile(target_vector, (len(thresholds), 1))
    


    #R_l
    r = feature_vector.size #size of R
    r_l = feature < thresholds
    num_r_l = np.sum(r_l, axis=1).reshape(-1, 1) #size of R_l
    index_deleted = np.where(num_r_l[:, 0] == 0)
    feature = np.delete(feature, index_deleted, 0)
    target = np.delete(target, index_deleted, 0)
    num_r_l = np.delete(num_r_l, index_deleted, 0)
    r_l = np.delete(r_l, index_deleted, 0)
    thresholds = np.delete(thresholds, index_deleted, 0)

    p_1_l = np.sum(np.multiply(r_l, target), axis=1).reshape(-1, 1)
    p_0_l = num_r_l - p_1_l
    p_1_l = np.divide(p_1_l, num_r_l)
    p_0_l = np.divide(p_0_l, num_r_l)
    
    num_r_l = np.divide(num_r_l, r)
    gini_l = -np.multiply(num_r_l, (1 - p_0_l ** 2 - p_1_l ** 2))


    #R_r
    r_r = feature >= thresholds
    num_r_r = np.sum(r_r, axis=1).reshape(-1, 1) #size of R_r
    index_deleted = np.where(num_r_r[:, 0] == 0)
    feature = np.delete(feature, index_deleted, 0)
    target = np.delete(target, index_deleted, 0)
    thresholds = np.delete(thresholds, index_deleted, 0)
    num_r_l = np.delete(num_r_l, index_deleted, 0)
    r_l = np.delete(r_l, index_deleted, 0)
    num_r_r = np.delete(num_r_r, index_deleted, 0)
    r_r = np.delete(r_r, index_deleted, 0)


    p_1_r = np.sum(np.multiply(r_r, target), axis=1).reshape(-1, 1)
    p_0_r = num_r_r - p_1_r
    p_1_r = np.divide(p_1_r, num_r_r)
    p_0_r = np.divide(p_0_r, num_r_r)

    num_r_r = np.divide(num_r_r, r)
    gini_r = -np.multiply(num_r_r, (1 - p_0_r ** 2 - p_1_r ** 2))

    #Result
    ginis = gini_r + gini_l
    ginis = ginis[:, 0]
    thresholds = thresholds[:, 0]
    index_min = np.argmin(ginis)
    indices = np.where(ginis == ginis[index_min]) 
    threshold_best = np.min(thresholds[indices])
...
class DecisionTree:
    ...
    def _fit_node(self, sub_X, sub_y, node):
        if np.all(sub_y == sub_y[0]): #CHANGED sub_y != sub_y[0]
            node["type"] = "terminal"
            node["class"] = sub_y[0]
            return
        ...
        for feature in range(1, sub_X.shape[1]):
            ...
            if feature_type == "real":
                ...
                counts = Counter(sub_X[:, feature]) #множество значений категориального признака    
                clicks = Counter(sub_X[sub_y == 1, feature])
                ratio = {}
                for key, current_count in counts.items():
                    if key in clicks: #key - one of the values of categorical feature
                        current_click = clicks[key] #part of 1 in every key
                    else:
                        current_click = 0
                    ratio[key] = current_click / current_count #CHANGED  current_count / current_click
                sorted_categories = list(map(lambda x: x[0], sorted(ratio.items(), key=lambda x: x[1])))#CHANGED lambda x: x[1]
            ...
            if gini_best is None or gini < gini_best: #CHANGED gini > gini_best  !!!!!!!!!!!!!!!!!!!!!
                ...
                if feature_type == "real":
                    ...
                    threshold_best = list(map(lambda x: x[1], #CHANGED lambda x: x[0]
    ...
    def _predict_node(self, x, node):
        ...
        if self._feature_types[node['feature_split']] == 'real':
            if x[node['feature_split']] < node['threshold']:
                self._redict_node(x, node['left_child'])
            else:
                self._redict_node(x, node['right_child'])
